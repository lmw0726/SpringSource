...
/**
 * {@code @ApplicationScope} 是 {@link Scope @Scope} 的一种专门化，用于将生命周期绑定到当前 Web 应用程序的组件。
 *
 * <p>具体来说，{@code @ApplicationScope} 是一个 <em>组合注解</em>，它充当了 {@code @Scope("application")} 的快捷方式，
 * 其默认 {@link #proxyMode} 设置为 {@link ScopedProxyMode#TARGET_CLASS TARGET_CLASS}。
 *
 * <p>{@code @ApplicationScope} 可以用作元注解，用于创建自定义的组合注解。
 *
 * @author Sam Brannen
 * @see RequestScope
 * @see SessionScope
